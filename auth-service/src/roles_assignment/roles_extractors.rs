...
use crate::{roles_assignment::{UserOdd, UserEven}, service::auth::{AuthTokenValidationError, Claims}};

#[derive(Debug, Clone)]
pub struct Authorized<T>(pub T);
impl<T, S: Send + Sync> OptionalFromRequestParts<S> for Authorized<T>
where
    Self: TryFrom<(Claims, String)>,
{
...
impl TryFrom<(Claims, String)> for Authorized<UserOdd> {
