...
impl EmailClient for PostmarkEmailClient {
    ...
    async fn send_email(&self, recipient: &Email, subject: &str, content: &Secret<String>) -> Result<()> {
        ...
        let request_body = SendEmailRequest {
            ...
            subject,
            html_body: content.expose_secret(),
            text_body: content.expose_secret(),
...
mod tests {
    ...
    fn content() -> Secret<String> {
        Secret::new(Paragraph(1..10).fake())
    }
    ...
    fn email_client(base_url: String) -> PostmarkEmailClient {
        ...
        let timeout = std::time::Duration::from_millis(auth_settings.email.timeout_milliseconds);
